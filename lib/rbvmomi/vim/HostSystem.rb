        ...
        @cached_dtm ||= begin
            RetrieveDynamicTypeManager()
                          ...
                          if summary.config.product.version >= "4.1.0"
                            if (summary.config.product.version < "5.0.0") && direct?
  ...
  module VIM
    class EsxcliNamespace
      ...
      def method_missing(name, *args)
        name = name.to_s
        if @namespaces.member?(name) && args.empty?
          @namespaces[name]
    ...
    class EsxcliCommand
      ...
      def call args={}
        if @ns.host.direct?
          ...
          args = args.reject { |k, _v| !real_args.member?(k.to_s) }
