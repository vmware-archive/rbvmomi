...
module RbVmomi
  module VIM
    class VirtualMachine
      ...
      def macs
        Hash[config.hardware.device.grep(RbVmomi::VIM::VirtualEthernetCard).map { |x| [x.deviceInfo.label, x.macAddress] }]
      end

      # Retrieve all virtual disk devices.
      # @return [Array] Array of virtual disk devices.
      def disks
        config.hardware.device.grep(RbVmomi::VIM::VirtualDisk)
      end
      ...
      def guest_ip
        g = guest
        g.ipAddress if g.ipAddress && (g.toolsStatus == "toolsOk" || g.toolsStatus == "toolsOld")
      end
