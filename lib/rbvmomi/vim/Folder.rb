    ...
    class Folder
      ...
      def traverse path, type=Object, create=false
        ...
        if x = p.find(final, type)
          x
        elsif create && (type == RbVmomi::VIM::Folder)
          p.CreateFolder(name: final)
        elsif create && (type == RbVmomi::VIM::Datacenter)
      ...
      def inventory_flat propSpecs={}
        propSet = [{ type: "Folder", pathSet: %w(name parent childEntity) }]
      ...
      def inventory_tree propSpecs={}
        ...
        rec = ->(parent) { Hash[(children[parent] || []).map { |x| [x, rec[x.obj]] }] }
        rec[self]
      end
