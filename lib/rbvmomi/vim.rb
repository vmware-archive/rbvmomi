...
module RbVmomi
  ...
  class VIM < Connection
    ...
    def self.connect opts
      ...
      opts[:user] ||= "root"
      opts[:password] ||= ""
      opts[:ssl] = true unless opts.member?(:ssl) || opts[:"no-ssl"]
      opts[:insecure] ||= false
      opts[:port] ||= (opts[:ssl] ? 443 : 80)
      opts[:path] ||= "/sdk"
      opts[:ns] ||= "urn:vim25"
      opts[:rev] = "6.7" if opts[:rev].nil?
      unless opts.member? :debug
        opts[:debug] = begin
                       !ENV["RBVMOMI_DEBUG"].empty?
    ...
    # Alias to service_content.rootFolder
    def rootFolder
    ...
    # Alias to service_content.propertyCollector
    def propertyCollector
      serviceContent.propertyCollector
    end

    # Alias to service_content.searchIndex
    def searchIndex
    ...
    def get_log_lines logKey, lines=5, start=nil, host=nil
      diagMgr = serviceContent.diagnosticManager
      unless start
        log = diagMgr.BrowseDiagnosticLog(host: host, key: logKey, start: 999_999_999)
        lineEnd = log.lineEnd
        start = lineEnd - lines
      end
      start = start < 0 ? 0 : start
      log = diagMgr.BrowseDiagnosticLog(host: host, key: logKey, start: start)
      if !log.lineText.empty?
