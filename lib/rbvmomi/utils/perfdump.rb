...
class PerfAggregator
  ...
  def add_node_unless_exists(inventory, id, props)
    inventory[id] = props.merge({ "children" => [] }) unless inventory[id]
  end
  ...
  def _compute_parents_and_children(objs)
    objs.each do |obj, props|
      ...
      props["parent_paths"] = {}
      next unless props["parent"]

      parent = objs[props["parent"]]
      props["paths"].each_key do |type|
  ...
  def _aggregate_metrics(vms_stats, perf_metrics)
    ...
    perf_metrics.each do |key, type|
      next unless type == :avg_ignore_zero || type == :avg
  ...
  def _collect_info_on_all_vms_single(root_folder, opts = {})
    ...
    hosts_props = inventory.select { |k, _v| k.is_a?(RbVmomi::VIM::HostSystem) }
    ...
    connected_vms = vms_props.select do |_vm, props|
    ...
    begin
      ...
      if e.fault.is_a? RbVmomi::VIM::ManagedObjectNotFound
        connected_vms -= [e.fault.obj]
        retry
      end
      raise
    end

    connected_hosts = hosts_props.select do |_k, v|
      v["runtime.connectionState"] != "disconnected"
    end
    unless connected_hosts.empty?
    ...
    hosts_props.each do |host, props|
      next unless connected_hosts[host]
    ...
    vms_props.each do |vm, props|
      next unless connected_vms.member?(vm)
      ...
      props["storage.space.committed"] = per_ds_usage.map(&:committed).inject(0, &:+)
      props["storage.space.uncommitted"] = per_ds_usage.map(&:uncommitted).inject(0, &:+)
      props["storage.space.unshared"] = per_ds_usage.map(&:unshared).inject(0, &:+)

      props["parent_paths"] = {}
      props["parent_paths"]["vmfolder"] = inventory[props["parent"]]["path"] if inventory[props["parent"]]
      unless props["config.template"]
        rp_props = inventory[props["resourcePool"]]
        props["parent_paths"]["rp"] = rp_props["path"]
      end

      props["annotation_yaml"] = YAML.safe_load(props["config.annotation"] || "")
      props["annotation_yaml"] = {} unless props["annotation_yaml"].is_a?(Hash)
  ...
  def _aggregate_vms(path_types, vms_props, inventory)
    ...
    path_types.each do |path_type|
      ...
      inventory.each do |k, v|
        next unless v["paths"] && v["paths"][path_type]
      ...
      paths_vms.each do |k, vms|
        ...
        vms_stats = vms_props.select { |k, _v| vms.member?(k) }.values
  ...
  def construct_tree_rows_from_vm_props(path_types = nil)
    ...
    Hash[path_types.map do |path_type|
      key, root = @inventory.find { |_k, v| v["paths"][path_type] == "root" }
