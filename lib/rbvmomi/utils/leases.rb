...
class LeaseTool
  ...
  def get_vms_props vms
    out = {}
    unless vms.empty?
      pc = vms.first._connection.service_content.propertyCollector
  ...
  def set_lease_in_vm_config vmconfig, lease_minutes
    ...
    annotation ||= ""
    note = YAML.safe_load annotation
    note = {} unless note.is_a?(Hash)
  ...
  def set_lease_on_leaseless_vms vms, vmprops, opts = {}
    ...
    raise "Expected lease_minutes to be specified" unless lease_minutes
    ...
    unless vms.empty?
      ...
      si = vms.first._connection.service_instance
