...
class CachedOvfDeployer
  ...
  def upload_ovf_as_template(ovf_url, template_name, opts = {})
    ...
    _run_without_interruptions(opts[:run_without_interruptions]) do
      begin
        vm = @vim.service_content.ovfManager.deployOVF(
          uri: ovf_url,
          vmName: vm_name,
        ...
        # template, see below ...
        is_duplicate = e.fault.is_a?(RbVmomi::VIM::DuplicateName)
        is_duplicate ||= begin
                           e.fault.is_a?(RbVmomi::VIM::InvalidState) &&
                             !e.fault.is_a?(RbVmomi::VIM::InvalidHostState)
                         end
        if is_duplicate
  ...
  def lookup_template(template_name)
    ...
    if template
      ...
      template = nil unless is_template
    end
    template
  end
