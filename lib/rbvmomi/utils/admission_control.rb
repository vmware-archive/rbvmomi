...
class AdmissionControlledResourceScheduler
  ...
  def initialize(vim, opts = {})
    ...
    @root_folder = @vim.service_content.rootFolder
  ...
  def vm_folder
    ...
    begin
      ...
      raise "VM folder #{@vm_folder_path} not found" unless @vm_folder
    rescue RbVmomi::Fault => e
      if !e.fault.is_a?(RbVmomi::VIM::DuplicateName)
        raise
      else
        retries -= 1
        retry if retries >= 0
  ...
  def datacenter
    unless @datacenter
      ...
      raise "datacenter #{@datacenter_path} not found" unless @datacenter
    end
    @datacenter
  end
  ...
  def datastores
    @datastores ||= @datastore_paths.map do |path|
      ...
      raise "datastore #{path} not found" unless ds
  ...
  def pods
    unless @pods
      ...
      @pods = computers.map do |computer, _stats|
        computersProps[computer]["datastore"] & datastores
      end.uniq.map do |ds_list|
        computers.map { |x| x[0] }.select do |computer|
          (computer.datastore & datastores) == ds_list
        end
      end
    end
    @pods
  ...
  def pod_vms(pod)
    ...
    filterSpec = RbVmomi::VIM.PropertyFilterSpec(
      objectSet: pod.map do |computer, _stats|
        {
    ...
    out.select { |obj, _props| obj.is_a?(RbVmomi::VIM::VirtualMachine) }
  end

  # Returns all candidate datastores for a given pod.
  # @return [Array] List of RbVmomi::VIM::Datastore
  def pod_datastores(pod)
    pod.first.datastore & datastores
  end
      ...
      @filtered_pods = pods.select do |pod|
        ...
        unless denied
          hosts_available = pod.any? do |computer|
            stats = Hash[computers][computer]
            stats[:totalCPU] > 0 && stats[:totalMem] > 0
          end
          unless hosts_available
  ...
  def pick_computer(placementhint = nil)
    unless @computer
      ...
      available = filtered_pods.flatten
      eligible = computers.select do |computer, stats|
        available.member?(computer) && stats[:totalCPU] > 0 and stats[:totalMem] > 0
      end
      computer = nil
      if placementhint
        computer = eligible.map { |x| x[0] }[placementhint % eligible.length] unless eligible.empty?
      else
        computer, = eligible.min_by do |_computer, stats|
          2**(stats[:usedCPU].to_f / stats[:totalCPU]) + (stats[:usedMem].to_f / stats[:totalMem])
        end
      end

      raise "No clusters available, should have been prevented by admission control" unless computer
  ...
  def datastore placementHint = nil
    ...
    raise "Couldn't find any eligible datastore. Admission control should have prevented this" if eligible.empty?

    @datastore = if placementHint && placementHint > 0
                   eligible[placementHint % eligible.length]
                 else
                   eligible.first
                 end
    @datastore
  ...
  def make_placement_decision opts = {}
    ...
    user_vms = vm_folder.inventory_flat("VirtualMachine" => %w(name storage)).select do |k, _v|
      k.is_a?(RbVmomi::VIM::VirtualMachine)
    end
    numVms = user_vms.length
    unshared = user_vms.map do |_vm, info|
      info["storage"].perDatastoreUsage.map(&:unshared).inject(0, &:+)
    end.inject(0, &:+)
    log "User stats: #{numVms} VMs using %.2fGB of storage" % [unshared.to_f / 1024**3]

    @placement_hint = opts[:placement_hint] || rand(1..100)
