...
module RbVmomi
  class NewDeserializer
    ...
    def deserialize(node, type = nil)
      ...
      # XXX what changed
      type_attr = node.attributes["type"].value if node.attributes["type"] && !type_attr
    ...
    def traverse_data(node, klass)
      ...
      klass.full_props_desc.each do |desc|
        ...
        while (child = children[i]) && (!klass.full_props_set.member? child.name)
          i += 1
        end
  ...
  class OldDeserializer
    ...
    def deserialize(xml, typename = nil)
      type_attr = if IS_JRUBY
                    xml.attribute_nodes.find do |a|
                      a.name == "type" &&
                        a.namespace &&
                        a.namespace.prefix == "xsi"
                    end
                  else
                    xml.attribute_with_ns("type", NS_XSI)
                  end
      ...
      warn "#{$ERROR_INFO.class} while deserializing #{xml.name} (#{typename}):"
      warn xml.to_s
      raise
    end
  ...
  Deserializer = if ENV["RBVMOMI_NEW_DESERIALIZER"] == "1" || true # Always use new one now
                   NewDeserializer
                 else
                   OldDeserializer
                 end
