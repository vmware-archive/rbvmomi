...
module RbVmomi
  ...
  class DeserializationFailed < RuntimeError; end
  ...
  class Connection < TrivialSoap
    ...
    def obj2xml(xml, name, type, is_array, o, attrs = {})
      expected = type(type)
      raise "expected array for '#{name}', got #{o.class.wsdl_name}" if is_array && !(o.is_a?(Array) || (o.is_a?(Hash) && (expected == BasicTypes::KeyValue)))

      case o
      when Array, BasicTypes::KeyValue
        if o.is_a?(BasicTypes::KeyValue) && (expected != BasicTypes::KeyValue)
          raise "expected #{expected.wsdl_name} for '#{name}', got KeyValue"
        elsif (expected == BasicTypes::KeyValue) && !is_array
      ...
      when BasicTypes::ManagedObject
        raise "expected #{expected.wsdl_name} for '#{name}', got #{o.class.wsdl_name} for field #{name.inspect}" if expected && (expected < o.class) && (expected != BasicTypes::AnyType)
      ...
      when BasicTypes::DataObject
        ...
        xml.tag! name, attrs.merge("xsi:type" => o.class.wsdl_name) do
          o.class.full_props_desc.each do |desc|
            next unless o.props.member? desc["name"].to_sym
      ...
      when Hash
        if (expected == BasicTypes::KeyValue) && is_array
      ...
      warn "#{$ERROR_INFO.class} while serializing #{name} (#{type}):"
      PP.pp o, $stderr
      raise
    end

    def self.type(name)
      raise unless name && (name.is_a?(String) || name.is_a?(Symbol))
