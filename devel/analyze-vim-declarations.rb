...
XML_FNS.each do |x|
  abort "XML file #{x} does not exist" unless File.exist? x
end

TYPES = {}.freeze
VERSIONS = [].freeze

ID2NAME = Hash.new { |_h, k| raise "unknown type-id #{k.inspect}" }
...
def handle_version(x)
  ...
  h["compatible"] = x.children.select(&:element?).map(&:text)
  VERSIONS << h
end
...
XML_FNS.each do |fn|
  ...
  xml_str = xml_str.gsub(%r{\<description-html\>(.*?)\</description-html\>}m, "")
