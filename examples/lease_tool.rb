...
VIM = RbVmomi::VIM
CMDS = %w(set_lease_on_leaseless_vms show_expired_vms
        show_soon_expired_vms kill_expired_vms).freeze
...
inventory = inventory.select { |obj, _props| obj.is_a?(VIM::VirtualMachine) }
case cmd
...
when "show_expired_vms"
  ...
  vms.each do |vm, _time_to_expiration|
